...
public class FileServerClientAuthHanderTest extends ChannelInboundHandlerAdapter{
	@Override
		...
		if(msg instanceof String ) {
			...
			if(StringUtils.isNotBlank(resHeaderStr)) {
				...
				if("1".equals(res_code)) {
					...
					ctx.channel().pipeline().remove("fileServerClientAuthHanderTest");
					System.out.println("移除认证授权Handler");
					ctx.channel().pipeline().addLast("clientFileUploadRequestHandlerTest", new ClientFileUploadRequestHandlerTest());
				}else {
